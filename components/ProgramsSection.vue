<template>
  <div class="bg-white py-12 sm:py-16 overflow-hidden">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      <!-- Section Header -->
      <div class="mx-auto max-w-2xl text-center mb-8">
        <h2 class="text-sm font-semibold leading-7 text-emerald-900 uppercase tracking-widest">Recent Milestone</h2>
        <p class="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">IDAC 2025 Book Launch</p>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
        <div v-for="(stat, index) in eventStats" :key="index" class="flex flex-col items-center p-4 bg-slate-50 rounded-xl">
          <dt class="text-xs font-medium text-gray-500">{{ stat.label }}</dt>
          <dd class="text-xl md:text-2xl font-bold text-emerald-900 tracking-tight">{{ stat.value }}</dd>
        </div>
      </div>
      ...
      <div class="mt-12 flex md:grid md:grid-cols-2 gap-6 overflow-x-auto snap-x snap-mandatory pb-4 scrollbar-hide">
        ...
        >
          ...
          <div>
            <h3 class="text-base font-bold text-gray-900 mb-1 truncate">{{ video.title }}</h3>
            <p class="text-sm text-gray-500 line-clamp-2">{{ video.description }}</p>
            <span class="mt-4 inline-flex items-center text-xs font-semibold text-emerald-900">
